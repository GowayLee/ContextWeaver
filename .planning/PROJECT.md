...
ContextWeaver 是一个面向 AI Agent 的代码语义检索上下文引擎，由本地 CLI 和可分发 Skill 资产组成。它为本地代码库提供索引、混合搜索、上下文扩展和 prompt 证据准备能力，当前工作重点是在保留既有能力的前提下继续补齐索引链路的可观测性、一致性与恢复语义。
...
- ✓ `index` 失败时会输出 provider 类型、HTTP 状态、错误分类与安全诊断摘要，帮助定位 batch size、provider 限制和认证类问题 — Phase 2
...
| 错误显示要补足上游响应与 batch size 诊断信息     | 社区已有频繁 issue，当前错误信息不足以支持自助排障                           | Done      |
...
_Last updated: 2026-04-01 after Phase 2 execution_
