...
- [ ] `index` 在 Embedding API 返回上游错误时立即停止后续向量化工作，不继续处理剩余批次
- [ ] `index` 失败时不再打印“索引完成”或误导性的成功统计，CLI 退出状态与索引真实结果保持一致
...
| ------------------------------------------------ | ---------------------------------------------------------------------------- | --------- |
| 本轮只聚焦 `index` 主链路                        | 当前最严重的问题出现在索引稳定性和状态一致性上，先修主路径最能恢复产品可信度 | — Pending |
| Embedding API 错误默认立即失败并退出             | 当前“继续跑 + 打印成功”会制造假成功状态，优先保证结果真实性                  | — Pending |
